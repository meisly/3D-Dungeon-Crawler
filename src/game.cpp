...
void Game::Run(Controller const &controller, Renderer &renderer,
  ...
  wchar_t *screen = new wchar_t[bufWidth*bufHeight];
  ...
  for (int x = 0; x < bufWidth; x++)
      {
        ...
        while (!bHitWall && fDistanceToWall < max_depth)
        {
          ...
          if (nTestX < 0 || nTestX >= map._width || nTestY < 0 || nTestY >= map._height)
          {
          ...
          else
          {
            ...
            if (map._string.c_str()[nTestX * map._width + nTestY] == '#')
            {
              // Ray has hit wall
              bHitWall = true;
        ...
        short nShade = ' ';
        if (fDistanceToWall <= max_depth / 4.0f)			nShade = 0x2588;	// Very close	
